"""Phone-36Train-Controlled-Tungro-15Epoch-Retry.

Controlled experimental retry training on the re-audited Tungro derived dataset.
This script:
- runs a fresh preflight audit on the re-audited derived dataset
- runs a new 15 epoch YOLO training
- validates baseline and controlled weights on the same derived val/test splits
- runs baseline and retry confidence sweep and failure analysis for Tungro
...
DERIVED_REBUILD_MANIFEST = DATASET_ROOT / "derived_rebuild_manifest.csv"
PRETRAIN_REAUDIT_REPORT = ROOT / "reports" / "phone_36_derived_dataset_rebuild_pretrain_reaudit" / "phone_36_derived_dataset_rebuild_pretrain_reaudit_report.md"
PRETRAIN_AUDIT_GATE_JSON = ROOT / "reports" / "phone_36_derived_dataset_rebuild_pretrain_reaudit" / "pretrain_audit_gate.json"
REBUILD_CONTEXT_JSON = ROOT / "reports" / "phone_36_derived_dataset_rebuild_pretrain_reaudit" / "rebuild_context.json"
...
RUN_NAME = "controlled_exp_phone_riceseg_v36_tungro_policy_fixed_reaudit_15epoch_retry"
RUN_DIR = RUN_PROJECT / RUN_NAME
REPORT_DIR = ROOT / "reports" / "phone_36_train_controlled_tungro_15epoch_retry"
REPORT_MD = REPORT_DIR / "phone_36_train_controlled_tungro_15epoch_retry_report.md"
TRAINING_CONTEXT_JSON = REPORT_DIR / "training_context.json"
PREFLIGHT_AUDIT_CSV = REPORT_DIR / "preflight_audit.csv"
...
BASELINE_COMPARISON_CSV = REPORT_DIR / "baseline_vs_retry_comparison.csv"
...
BASELINE_TUNGRO_CONF_DIRS = {
    0.05: REPORT_DIR / "baseline_predictions_tungro_conf_005",
    0.10: REPORT_DIR / "baseline_predictions_tungro_conf_010",
    0.25: REPORT_DIR / "baseline_predictions_tungro_conf_025",
}
...
def scan_dataset(data_yaml: dict[str, Any]) -> dict[str, Any]:
    ...
    reaudit_data_yaml_pass = yaml_path_resolved == DATASET_ROOT.resolve()
    ...
    reaudit_label_sanitation_pass = (
        ...
        and sum(count for key, count in sanitation_counter.items() if key.startswith("bad_field_count")) == 0
        and sum(count for key, count in sanitation_counter.items() if key.startswith("non_numeric")) == 0
        and sanitation_counter.get("missing_label_count", 0) == 0
        and sanitation_counter.get("missing_image_count", 0) == 0
    )

    return {
        "reaudit_data_yaml_pass": bool(reaudit_data_yaml_pass and name_pass),
        ...
        "reaudit_label_sanitation_pass": reaudit_label_sanitation_pass,
...
def assert_required_paths() -> None:
    required = [
        DATASET_ROOT,
        DATA_YAML,
        DERIVED_REBUILD_MANIFEST,
        PRETRAIN_REAUDIT_REPORT,
        PRETRAIN_AUDIT_GATE_JSON,
        REBUILD_CONTEXT_JSON,
        BASELINE_RUN_DIR,
        BASELINE_WEIGHTS,
        BASELINE_DATA_YAML,
        BASELINE_ARGS_YAML,
...
def write_preflight_audit(
    *,
    data_yaml_path_value: str,
    data_yaml_path_resolved: str,
    reaudit_data_yaml_pass: bool,
    tungro_class_id_confirmed: bool,
    bbox_out_of_bounds_count: int,
    class_id_out_of_range_count: int,
    non_positive_width_height_count: int,
    ...
    rows = [
        {"check_name": "reaudit_data_yaml_path_value", "status": "PASS" if data_yaml_path_value else "FAIL", "value": data_yaml_path_value, "notes": ""},
        {"check_name": "reaudit_data_yaml_path_resolved", "status": "PASS" if data_yaml_path_resolved else "FAIL", "value": data_yaml_path_resolved, "notes": ""},
        {"check_name": "reaudit_data_yaml_pass", "status": "PASS" if reaudit_data_yaml_pass else "FAIL", "value": reaudit_data_yaml_pass, "notes": ""},
        {"check_name": "tungro_class_id_confirmed", "status": "PASS" if tungro_class_id_confirmed else "FAIL", "value": tungro_class_id_confirmed, "notes": "expected class_id 3"},
        {"check_name": "bbox_out_of_bounds_count", "status": "PASS" if bbox_out_of_bounds_count == 0 else "FAIL", "value": bbox_out_of_bounds_count, "notes": ""},
        {"check_name": "class_id_out_of_range_count", "status": "PASS" if class_id_out_of_range_count == 0 else "FAIL", "value": class_id_out_of_range_count, "notes": ""},
        {"check_name": "non_positive_width_height_count", "status": "PASS" if non_positive_width_height_count == 0 else "FAIL", "value": non_positive_width_height_count, "notes": ""},
        {"check_name": "parse_error_count", "status": "PASS" if parse_error_count == 0 else "FAIL", "value": parse_error_count, "notes": ""},
        {"check_name": "missing_label_count", "status": "PASS" if missing_label_count == 0 else "FAIL", "value": missing_label_count, "notes": ""},
        {"check_name": "missing_image_count", "status": "PASS" if missing_image_count == 0 else "FAIL", "value": missing_image_count, "notes": ""},
        {"check_name": "val_tungro_images", "status": "PASS" if split_summaries["val"]["tungro_images"] > 0 else "FAIL", "value": split_summaries["val"]["tungro_images"], "notes": ""},
        {"check_name": "test_tungro_images", "status": "PASS" if split_summaries["test"]["tungro_images"] > 0 else "FAIL", "value": split_summaries["test"]["tungro_images"], "notes": ""},
    ]
    atomic_write_csv(PREFLIGHT_AUDIT_CSV, rows, ["check_name", "status", "value", "notes"])


def write_gate_json(training_context: dict[str, Any]) -> None:
    payload = {
        "phone_36_train_controlled_tungro_15epoch_retry_gate": training_context["phone_36_train_controlled_tungro_15epoch_retry_gate"],
        "reaudit_data_yaml_pass": training_context["reaudit_data_yaml_pass"],
        "reaudit_label_sanitation_pass": training_context["reaudit_label_sanitation_pass"],
        "tungro_eval_splits_ready": training_context["tungro_eval_splits_ready"],
        "used_reaudit_dataset": training_context["used_reaudit_dataset"],
        "used_old_mini_dataset": training_context["used_old_mini_dataset"],
    ...
    atomic_write_json(GATE_JSON, payload)
...
def write_report(
    ...
    val_row = next((row for row in overall_rows if row["model_name"] == "retry_15epoch" and row["dataset_split"] == "val"), None)
    test_row = next((row for row in overall_rows if row["model_name"] == "retry_15epoch" and row["dataset_split"] == "test"), None)
    baseline_test_row = next((row for row in overall_rows if row["model_name"] == "baseline_35N" and row["dataset_split"] == "test"), None)
    retry_tungro_val = next((row for row in tungro_rows if row["model_name"] == "retry_15epoch" and row["dataset_split"] == "val"), None)
    retry_tungro_test = next((row for row in tungro_rows if row["model_name"] == "retry_15epoch" and row["dataset_split"] == "test"), None)
    baseline_tungro_val = next((row for row in tungro_rows if row["model_name"] == "baseline_35N" and row["dataset_split"] == "val"), None)
    baseline_tungro_test = next((row for row in tungro_rows if row["model_name"] == "baseline_35N" and row["dataset_split"] == "test"), None)

    report = f'''# Phone-36Train-Controlled-Tungro-15Epoch-Retry Report
...
- Used reaudit derived dataset: `{"YES" if training_context["used_reaudit_dataset"] else "NO"}`
- Used old mini derived dataset: `{"YES" if training_context["used_old_mini_dataset"] else "NO"}`
...
- reaudit_dataset_root: `{training_context["reaudit_dataset_root"]}`
- reaudit_data_yaml_path: `{training_context["reaudit_data_yaml_path"]}`
- pretrain_reaudit_report_path: `{training_context["pretrain_reaudit_report_path"]}`
- pretrain_audit_gate_json_path: `{training_context["pretrain_audit_gate_json_path"]}`
- rebuild_context_json_path: `{training_context["rebuild_context_json_path"]}`
- derived_rebuild_manifest_path: `{training_context["derived_rebuild_manifest_path"]}`
...
- new_run_dir: `{training_context["new_run_dir"]}`
...
- train_tungro_images: `{training_context["split_summaries"]["train"]["tungro_images"]}`
- val_tungro_images: `{training_context["split_summaries"]["val"]["tungro_images"]}`
- test_tungro_images: `{training_context["split_summaries"]["test"]["tungro_images"]}`
- train_tungro_bboxes: `{training_context["split_summaries"]["train"]["tungro_bboxes"]}`
- val_tungro_bboxes: `{training_context["split_summaries"]["val"]["tungro_bboxes"]}`
- test_tungro_bboxes: `{training_context["split_summaries"]["test"]["tungro_bboxes"]}`
...
- epochs_completed: `{training_context.get("epochs_completed", 0)}`
- best_epoch_by_map50: `{best_epoch}`
- best_epoch_map50: `{best_map50}`
- train_start_time: `{training_context.get("train_start_time", "MISSING")}`
- train_end_time: `{training_context.get("train_end_time", "MISSING")}`

## Validation Snapshot

- retry val precision: `{val_row["precision"] if val_row else "MISSING"}`
- retry val recall: `{val_row["recall"] if val_row else "MISSING"}`
- retry val mAP50: `{val_row["mAP50"] if val_row else "MISSING"}`
- retry val mAP50-95: `{val_row["mAP50_95"] if val_row else "MISSING"}`
- retry test precision: `{test_row["precision"] if test_row else "MISSING"}`
- retry test recall: `{test_row["recall"] if test_row else "MISSING"}`
- retry test mAP50: `{test_row["mAP50"] if test_row else "MISSING"}`
- retry test mAP50-95: `{test_row["mAP50_95"] if test_row else "MISSING"}`
- train_metrics_summary_csv: `{TRAIN_METRICS_SUMMARY_CSV.resolve()}`
- per_class_metrics_csv: `{PER_CLASS_METRICS_CSV.resolve()}`

## Baseline Comparison

- historical 35N experimental mAP50: `0.38768203615854735`
- historical 35N Tungro per-class AP: `0.11919959830149554`
- strict baseline test mAP50 on reaudit dataset: `{baseline_test_row["mAP50"] if baseline_test_row else "MISSING"}`
- strict baseline Tungro AP50 on reaudit test: `{baseline_tungro_test["tungro_AP50"] if baseline_tungro_test else "MISSING"}`
- baseline_vs_retry_comparison_csv: `{BASELINE_COMPARISON_CSV.resolve()}`

## Tungro Outcome

- retry test Tungro precision: `{retry_tungro_test["tungro_precision"] if retry_tungro_test else "MISSING"}`
- retry test Tungro recall: `{retry_tungro_test["tungro_recall"] if retry_tungro_test else "MISSING"}`
- retry test Tungro AP50: `{retry_tungro_test["tungro_AP50"] if retry_tungro_test else "MISSING"}`
- retry test Tungro AP50-95: `{retry_tungro_test["tungro_AP50_95"] if retry_tungro_test else "MISSING"}`
- retry val Tungro no-detection count @ conf=0.25: `{retry_tungro_val["num_no_detection_cases"] if retry_tungro_val else "MISSING"}`
- baseline val Tungro no-detection count @ conf=0.25: `{baseline_tungro_val["num_no_detection_cases"] if baseline_tungro_val else "MISSING"}`
- retry test Tungro no-detection count @ conf=0.25: `{retry_tungro_test["num_no_detection_cases"] if retry_tungro_test else "MISSING"}`
...
- note: `test` no-detection improved at conf=0.25, while `val` no-detection rose; conf sweep should be read as a calibration signal, not as a formal deployment claim.
- tungro_eval_summary_csv: `{TUNGRO_EVAL_SUMMARY_CSV.resolve()}`

## Confidence Sweep

- tungro_conf_sweep_completed: `{training_context["tungro_conf_sweep_completed"]}`
- prediction_conf_sweep_csv: `{CONF_SWEEP_CSV.resolve()}`
- saved retry prediction dirs:
...
- saved baseline prediction dirs:
  - `{BASELINE_TUNGRO_CONF_DIRS[0.05].resolve()}`
  - `{BASELINE_TUNGRO_CONF_DIRS[0.10].resolve()}`
  - `{BASELINE_TUNGRO_CONF_DIRS[0.25].resolve()}`

## Failure Analysis

- failure_cases_analyzed: `{training_context["failure_cases_analyzed"]}`
- failure_case_count: `{len(failure_rows)}`
- failure_cases_csv: `{FAILURE_CASES_CSV.resolve()}`
...
1. ???????? `phone_riceseg_v36_tungro_policy_fixed_reaudit`?`{"YES" if training_context["used_reaudit_dataset"] else "NO"}`
2. ????? old mini ????`{"YES" if training_context["used_old_mini_dataset"] else "NO"}`
3. reaudit data.yaml ?????`{"YES" if training_context["reaudit_data_yaml_pass"] else "NO"}`
4. reaudit labels ???? bbox ???`{"YES" if training_context["bbox_out_of_bounds_count"] > 0 else "NO"}`
5. val/test Tungro ????????`{"YES" if training_context["tungro_eval_splits_ready"] else "NO"}`
6. ?????? 15 epoch controlled training?`{"YES" if training_context["training_run_completed"] else "NO"}`
7. ??????????`{"YES" if training_context["best_pt_exists"] else "NO"}`
8. ????????`NO`
9. Tungro AP50 ?????`{training_context["tungro_AP50_improved"]}`
10. Tungro recall ?????`{training_context["tungro_recall_improved"]}`
11. Tungro NO_DETECTION ?????`{training_context["tungro_no_detection_reduced"]}`
12. ??????????`{"YES" if training_context["overfitting_risk"] == "high" else "NO"}`
13. ???? baseline ??????`{"YES" if training_context["baseline_comparison_completed"] else "NO"}`
14. ???? Tungro failure case ???`{"YES" if training_context["failure_cases_analyzed"] else "NO"}`
15. ????????? full controlled training?`{"YES" if training_context["allow_next_full_training"] else "NO"}`
16. ???? backend demo claim?`NO`
17. ???? candidate claim?`NO`

## Final Boundary Sentence

????? `phone_riceseg_v36_tungro_policy_fixed_reaudit` ? 15 epoch ?????????????????? bbox ???????? Tungro ?????????????? backend demo claim???? candidate claim?
'''
...
def main() -> int:
    ...
    bbox_out_of_bounds_count = sum(count for key, count in scan_result["sanitation_issues"].items() if key.startswith("bbox_out_of_bounds"))
    class_id_out_of_range_count = sum(count for key, count in scan_result["sanitation_issues"].items() if key.startswith("class_id_out_of_range"))
    non_positive_width_height_count = sum(count for key, count in scan_result["sanitation_issues"].items() if key.startswith("non_positive_width_height"))
    parse_error_count = (
        sum(count for key, count in scan_result["sanitation_issues"].items() if key.startswith("bad_field_count"))
        + sum(count for key, count in scan_result["sanitation_issues"].items() if key.startswith("non_numeric"))
    )
    ...
    write_preflight_audit(
        data_yaml_path_value=scan_result["yaml_path_value"],
        data_yaml_path_resolved=scan_result["yaml_path_resolved"],
        reaudit_data_yaml_pass=scan_result["reaudit_data_yaml_pass"],
    ...
    training_context: dict[str, Any] = {
        ...
        "reaudit_dataset_root": str(DATASET_ROOT.resolve()),
        "reaudit_data_yaml_path": str(DATA_YAML.resolve()),
        "reaudit_train_images_path": str((DATASET_ROOT / "images" / "train").resolve()),
        "reaudit_train_labels_path": str((DATASET_ROOT / "labels" / "train").resolve()),
        "reaudit_val_images_path": str((DATASET_ROOT / "images" / "val").resolve()),
        "reaudit_val_labels_path": str((DATASET_ROOT / "labels" / "val").resolve()),
        "reaudit_test_images_path": str((DATASET_ROOT / "images" / "test").resolve()),
        "reaudit_test_labels_path": str((DATASET_ROOT / "labels" / "test").resolve()),
        "pretrain_reaudit_report_path": str(PRETRAIN_REAUDIT_REPORT.resolve()),
        "pretrain_audit_gate_json_path": str(PRETRAIN_AUDIT_GATE_JSON.resolve()),
        "rebuild_context_json_path": str(REBUILD_CONTEXT_JSON.resolve()),
        "derived_rebuild_manifest_path": str(DERIVED_REBUILD_MANIFEST.resolve()),
        ...
        "new_run_dir": str(RUN_DIR.resolve()),
        ...
        "used_reaudit_dataset": True,
        "used_old_mini_dataset": False,
        "reaudit_data_yaml_pass": scan_result["reaudit_data_yaml_pass"],
        "tungro_class_id_confirmed": scan_result["tungro_class_id_confirmed"],
        "reaudit_label_sanitation_pass": scan_result["reaudit_label_sanitation_pass"],
        ...
        "block_reasons": [],
        "phone_36_train_controlled_tungro_15epoch_retry_gate": "BLOCKED",
    ...
    if bbox_out_of_bounds_count > 0:
        training_context["block_reasons"].append(f"reaudit_bbox_out_of_bounds_count={bbox_out_of_bounds_count}")
    if parse_error_count > 0:
        training_context["block_reasons"].append(f"reaudit_parse_error_count={parse_error_count}")
    if not training_context["tungro_eval_splits_ready"]:
        training_context["block_reasons"].append(
            "reaudit_tungro_eval_splits_empty: val_tungro_images=0 and/or test_tungro_images=0"
        )
    if (
        not training_context["reaudit_data_yaml_pass"]
        or not training_context["reaudit_label_sanitation_pass"]
        ...
        write_gate_json(training_context)
        write_report(training_context, [], [], [], [], [], [], [])
        return 2

    training_context["train_start_time"] = now_iso()
    best_weights = run_train(device=device)
    training_context["train_end_time"] = now_iso()
    ...
    training_context["epochs_completed"] = len(train_results_rows)
    ...
    controlled_overall_val, controlled_per_val = validate_model(
        ...
        split="val",
        device=device,
        name_suffix="retry_15epoch_val_on_v36_derived",
    )
    controlled_overall_test, controlled_per_test = validate_model(
        ...
        split="test",
        device=device,
        name_suffix="retry_15epoch_test_on_v36_derived",
    )
    ...
    for conf in CONF_LIST:
        ...
        for split, stats in per_split_stats.items():
            controlled_conf_stats_by_split[("retry_15epoch", split, conf)] = stats

    baseline_conf_stats_by_split: dict[tuple[str, str], dict[str, Any]] = {}
    for conf in CONF_LIST:
        baseline_results = run_predict_save(
            baseline_model,
            [str(path) for path in all_tungro_images],
            conf,
            BASELINE_TUNGRO_CONF_DIRS[conf],
            device,
        )
        baseline_rows, baseline_per_split_stats, _ = summarize_prediction_results(
        ...
        conf_rows.extend(baseline_rows)
        for split, stats in baseline_per_split_stats.items():
            baseline_conf_stats_by_split[("baseline_35N", split, conf)] = stats

    for split in ("val", "test"):
        conf025 = controlled_conf_stats_by_split[("retry_15epoch", split, 0.25)]
        conf005 = controlled_conf_stats_by_split[("retry_15epoch", split, 0.05)]
        low_conf_cases = max(0, conf025["num_no_detection_cases"] - conf005["num_no_detection_cases"])
        controlled_conf_stats_by_split[("retry_15epoch", split)] = {
            **conf025,
            "num_low_conf_cases": low_conf_cases,
            "notes": "strict comparison on derived dataset at conf=0.25; low_conf from conf0.05 delta",
        }
        baseline_conf025 = baseline_conf_stats_by_split[("baseline_35N", split, 0.25)]
        baseline_conf005 = baseline_conf_stats_by_split[("baseline_35N", split, 0.05)]
        baseline_low_conf_cases = max(0, baseline_conf025["num_no_detection_cases"] - baseline_conf005["num_no_detection_cases"])
        baseline_conf_stats_by_split[("baseline_35N", split)] = {
            **baseline_conf025,
            "num_low_conf_cases": baseline_low_conf_cases,
            "notes": "strict comparison on derived dataset at conf=0.25; low_conf from conf0.05 delta",
        }
    ...
    for split in ("val", "test"):
        all_tungro_eval_stats[("retry_15epoch", split)] = controlled_conf_stats_by_split[("retry_15epoch", split)]
    ...
    controlled_lookup = {(row["model_name"], row["dataset_split"]): row for row in overall_rows if row["model_name"] == "retry_15epoch"}
    baseline_tungro_lookup = {(row["model_name"], row["dataset_split"]): row for row in tungro_rows if row["model_name"] == "baseline_35N"}
    controlled_tungro_lookup = {(row["model_name"], row["dataset_split"]): row for row in tungro_rows if row["model_name"] == "retry_15epoch"}

    for split in ("val", "test"):
        b_overall = baseline_lookup[("baseline_35N", split)]
        c_overall = controlled_lookup[("retry_15epoch", split)]
            ...
            comparison_rows.append(
                {
                    ...
                    "retry_15epoch_value": controlled_value,
        ...
        c_tungro = controlled_tungro_lookup[("retry_15epoch", split)]
    ...
    atomic_write_csv(
        ...
        comparison_rows,
        ["metric", "baseline_value", "retry_15epoch_value", "delta", "comparison_dataset", "comparison_not_strict", "notes"],
    )
    ...
    pass_conditions = all(
        [
            training_context["reaudit_data_yaml_pass"],
            training_context["reaudit_label_sanitation_pass"],
            ...
            (training_context["tungro_no_detection_reduced"] or controlled_tungro_lookup[("retry_15epoch", "test")]["num_no_detection_cases"] <= 1),
    ...
    training_context["phone_36_train_controlled_tungro_15epoch_retry_gate"] = gate
