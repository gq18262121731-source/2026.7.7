...
> 对外发送给移动端、大屏或主系统团队时，建议使用 `docs/integration_package/` 下的联调接口包；本文件是全量接口合同，更适合后端和内部总览。
...
| 通道 | 用途 | 事件 |
|---|---|---|
| `ws://127.0.0.1:8000/ws/results` | 推送单图/跟拍识别结果 | `detection_result` |
| `ws://127.0.0.1:8000/ws/tasks` | 推送批量任务进度 | `task_status` |
| `ws://127.0.0.1:8000/ws/alerts` | 推送告警变化 | `alert_event` |
...
```bash
curl -X POST "http://127.0.0.1:8000/api/detect/batch" \
...
```bash
curl -X POST "http://127.0.0.1:8000/api/uav/tasks" \
  -H "Content-Type: application/json" \
  -d '{
    "field_id": "field_001",
...
curl -X POST "http://127.0.0.1:8000/api/uav/tasks/{uav_task_id}/dry-run" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "http://127.0.0.1:8000/api/risk/fusion/evaluate" \
  -H "Content-Type: application/json" \
  -d '{
    "field_id": "field_001",
