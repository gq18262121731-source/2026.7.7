...
router = APIRouter(prefix="/api/farm-analysis-reports", tags=["farm-analysis-reports"])


@router.post("/generate", response_model=FarmAnalysisReportResponse)
async def generate_farm_analysis_report(payload: FarmAnalysisReportRequest) -> FarmAnalysisReportResponse:
    return farm_analysis_report_service.generate_report(payload)


@router.get("", response_model=FarmAnalysisReportHistoryResponse)
async def farm_analysis_report_history(plot_id: str | None = None) -> FarmAnalysisReportHistoryResponse:
    items = [
        FarmAnalysisReportHistoryItem(
...
async def download_farm_analysis_report(report_id: str) -> FileResponse:
    return _report_file(report_id, "attachment")


@router.get("/{report_id}/preview")
async def preview_farm_analysis_report(report_id: str) -> FileResponse:
