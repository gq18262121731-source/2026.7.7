...
当前演示环境部分 FastAPI 默认错误可能仍是 `detail` 结构。移动端第一阶段需要兼容两种错误格式：

1. 统一错误结构：`success/error_code/message/detail`
2. FastAPI 默认结构：`detail`

对外部署时建议统一转换为 `success/error_code/message/detail` 结构。
...
- 不要直接裸显示英文异常栈。
- 优先读取 `error_code` 和 `message`；如果响应只有 `detail`，应降级展示为“请求参数不完整或格式错误”等中文提示。
- 404 类错误展示空态或返回上一级。
