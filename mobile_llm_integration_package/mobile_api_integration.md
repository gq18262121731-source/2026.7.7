...
该接口不是问答接口，不生成“白叶枯怎么判断”这类回答；它只告诉前端当前智能问答能力是否可用。真正回答用户问题使用 `POST /api/agent/diagnosis-report`，知识依据聚合使用 `POST /api/agent/knowledge-context`。
...
-> 根据状态展示 UI 提示
-> 用户输入真实问题
-> POST /api/agent/knowledge-context 获取知识依据
-> POST /api/agent/diagnosis-report 生成辅助回答
```
...
- LLM 状态只用于判断智能问答能力，不用于判断识别模型能力。
- LLM 不可用时仍允许展示识别图片、识别结果、风险等级、建议和免责声明。
- `mock_fallback_enabled=true` 只表示后端允许失败后降级 mock，不代表当前回答已经降级。
- 某次回答是否降级，要看 `POST /api/agent/diagnosis-report` 响应中的 `fallback_used`、`fallback_level`、`llm_mode`、`api_error_type` 等字段。
...
| 字段 | 含义 | 前端使用建议 |
|---|---|---|
| `llm_mode` | 当前 LLM 运行模式 | 判断是真实 LLM、mock 还是其他降级模式 |
| `llm_provider` | 当前大模型服务商 | 可在调试页展示 |
| `llm_model` | 当前模型名称 | 可在调试页展示 |
| `api_key_configured` | 是否配置 API Key | `false` 时真实 LLM 基本不可用 |
| `mock_fallback_enabled` | 是否允许失败后降级 mock | 只表示允许降级，不表示已经降级 |
| `json_response_format_enabled` | 是否要求模型返回结构化 JSON | 前端一般不用处理 |
| `prompt_version` | 当前提示词版本 | 用于问题追踪和调试 |
...
- 上传前检查文件类型和大小。
- 移动端代码使用 FormData 上传图片时，建议让 HTTP 客户端自动生成 `Content-Type` 和 boundary；不要手动拼接 base64，也不要手动固定 boundary。
- 返回 `record_id` 后进入识别结果详情页。
- 单图识别成功后，请把响应中的 `record_id` 写入 Postman 变量或移动端状态，再请求 `/api/mobile/records/{record_id}`、`/api/mobile/suggestions/{record_id}`、`/api/agent/knowledge-context` 和 `/api/agent/diagnosis-report`。
...
| `model_class` | 否 | string | 模型输出类别；移动端不要写死 UAV 类别，优先使用识别结果中的 `class_code` / `class_name` / `label`，或使用 `disease_id` 映射结果 |
...
第二阶段接口已移至 `mobile_api_phase2_optional.md`，不属于移动端第一阶段必接范围。
